...
They don't specify what data they use other than saying it is publicly available and they do not use any proprietary data from Meta's services. THe model was pre-trained on 2 trillion tokens. TODO: Compare with ChatGPT and other models
...
We may be entering an era where supervised data (human generated text) for supervised fine-tuning is no longer as useful as model-generated text from the most powerful models like GPT-4. Andrej Karpathy made a similar argument in his talk ([State of GPT at around 17:00](https://youtu.be/bZQun8Y4L2A?t=1069)) saying that its easier for humans to compare  two completions (what one does when creating training data for the reward model) vs. creating a completion from scratch (what one does when creating training data for the SFT model).
